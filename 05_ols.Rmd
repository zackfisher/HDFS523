# Ordinary Least Squares Review {#chapter-5}
...
So here we have no predictors, what is $\beta_0$? Here, $\beta_0$ is the mean of the response variable, and we can show this with some algebra,
...
Now, since $\boldsymbol{X'}\boldsymbol{X}$ is a square matrix and preseeumabely has an inverse (e.g. no perfect collinearity), we can premultiply both sides by $\left(\boldsymbol{X'}\boldsymbol{X}\right)^{-1}$, to obtain
