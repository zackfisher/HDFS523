...
Let's add a predictor to our model. In this case we will add
...
We can extraxt the fixed effects for a prototypical trajectory. 
...
**Remapping time**    
assessment = Number of assessments youth have been exposed to: assessment = 1, 2, 3, 4
Note: the way the mapping is done here only works in this case with no missing data. If have missing data, need to remap in a different way.
```{r}
...
Always be very careful with interpreting the time metric.
